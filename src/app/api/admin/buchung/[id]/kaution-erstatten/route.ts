...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (kundeId) {
      ...
      if (action === "voll") {
        subject = `Kaution Buchung #${buchungId} — voll zurueckerstattet`;
        mailBody = `Hallo ${kundeName},\n\nIch habe die Artikel geprueft — alles in Ordnung. Ihre Kaution wird in voller Hoehe (${kautionSoll.toFixed(2)} EUR) zurueckerstattet:\n\n${piId ? "- Stripe-Hold wurde freigegeben — kein Betrag wurde abgebucht." : "- Die Erstattung ueberweise ich Ihnen in den naechsten Werktagen."}\n\nVielen Dank fuer Ihre Buchung — wir freuen uns auf den naechsten Termin!\n\nMit freundlichen Gruessen\nManuel Buettner — Eventverleih Bergstrasse`;
      } else if (action === "teil") {
        subject = `Kaution Buchung #${buchungId} — Teilerstattung wegen Schaden`;
        mailBody = `Hallo ${kundeName},\n\nLeider gab es bei der Pruefung der Artikel einen Schaden. Schadensbetrag: ${schadenEur.toFixed(2)} EUR.\n\nDamit wird ein Teil Ihrer Kaution einbehalten:\n  Kaution gesamt: ${kautionSoll.toFixed(2)} EUR\n  Einbehalten:    ${schadenEur.toFixed(2)} EUR\n  Erstattung:     ${kautionRueckzahlungEur.toFixed(2)} EUR\n\n${body.schaden_notiz ? `Schaden-Notiz: ${body.schaden_notiz}\n\n` : ""}${piId ? "Die Erstattung erfolgt automatisch ueber Stripe in den naechsten 5 Werktagen." : "Die Erstattung ueberweise ich Ihnen in den naechsten Werktagen."}\n\nBei Rueckfragen melden Sie sich gerne per WhatsApp/Tel +49 156 79521124.\n\nMit freundlichen Gruessen\nManuel Buettner — Eventverleih Bergstrasse`;
      } else {
        subject = `Kaution Buchung #${buchungId} — kompletter Einzug wegen Schaden`;
        mailBody = `Hallo ${kundeName},\n\nbei der Pruefung der Artikel gab es einen Schaden, dessen Hoehe die Kaution erreicht oder uebersteigt. Daher wird die Kaution komplett einbehalten.\n\nKaution einbehalten: ${kautionSoll.toFixed(2)} EUR\n${body.schaden_notiz ? `\nSchaden-Notiz: ${body.schaden_notiz}\n` : ""}\nBei Rueckfragen oder Klaerungsbedarf melden Sie sich bitte direkt bei mir: WhatsApp/Tel +49 156 79521124.\n\nMit freundlichen Gruessen\nManuel Buettner — Eventverleih Bergstrasse`;
      }
