 ...
 *
 * Reihenfolge nach Relevanz (Manuel nutzt die Seite zum Nachschlagen):
 *  1. Auf-einen-Blick-Kopf (Kunde · Event · Termine · Zahlungs-Ampel · Klartext-Status)
 *  2. Primäre Aktion (status-abhängig: Übergabe / Rückgabe / Kaution abrechnen / Termin)
 *  3. Bestellung (Artikel)
 *  4. Zahlungen (Übersicht + Zahlungseingang erfassen zusammen)
 *  5. Checkliste · Rechnungen · Schriftverkehr · Notizen · Sekundär-Aktionen (eingeklappt)
 */
...
const numv = (v: string | null | undefined): number => parseFloat(v ?? "0") || 0;
...
const TON_CLASS: Record<StatusTon, string> = {
  ...
  neutral: "bg-warm-bg text-warm-muted border-warm-border",
...
export default async function BuchungDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sk = statusKlartext(buchung);

  // Zahlungs-Übersicht (für Glance-Ampel + Zahlungs-Sektion)
  const gesamt =
    numv(buchung.Preis_Artikel) +
    numv(buchung.Preis_Lieferung) +
    numv(buchung.Preis_Abholung) +
...
Vertrag
      ...
      <section className="p-5 rounded-xl bg-warm-surface border border-warm-border">
        ...
        <div className={`mt-4 inline-block px-3 py-1.5 rounded-lg border text-sm font-medium ${TON_CLASS[sk.ton]}`}>
          {sk.text}
        </div>

        <dl className="mt-4 grid sm:grid-cols-3 gap-3 text-sm">
          <div>
            <dt className="text-xs uppercase tracking-wider text-warm-muted">Event</dt>
            <dd className="text-warm-text mt-0.5">{eventRange}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wider text-warm-muted">Übergabe</dt>
            <dd className="text-warm-text mt-0.5">{fmtDateTime(buchung.Uebergabe_Termin)}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wider text-warm-muted">Rückgabe</dt>
            <dd className="text-warm-text mt-0.5">{fmtDateTime(buchung.Rueckgabe_Termin)}</dd>
          </div>
        </dl>
      </section>

      {/* 2. Primäre Aktion (status-abhängig) */}
      ...
      {/* Kaution-Prüfphase: nach Rückgabe sichtbar wenn Kaution offen */}
      ...
      {/* Termine (Übergabe + Rückgabe) — Sync mit Google Calendar wenn ENV gesetzt */}
      <TerminePanel
        buchungId={buchung.id}
        ...
        <table className="w-full text-sm">
          <tbody>
            ...
            <tr className="border-t border-warm-border font-semibold">
              ...
              <td className="text-right font-mono text-warm-text">{fmtEur(gesamt.toFixed(2))}</td>
            </tr>
            <tr>
      ...
      {!["Storniert", "Abgerechnet", "Zurueckgegeben"].includes(status) && (
        <details>
          ...
          <section className="mt-2 p-5 rounded-xl bg-warm-surface border border-warm-border">
            {(async () => {
              const settings = await loadEveSettings();
              const bezahltSt = bezahltEur(buchung);
